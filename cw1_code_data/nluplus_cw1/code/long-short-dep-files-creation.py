...
def split_dependencies(file_path):
    ...
    short_dep = df[df['dependency_distance'] < 5]
    medium_dep = df[(df['dependency_distance'] >= 5) & (df['dependency_distance'] <= 10)]
    long_dep = df[df['dependency_distance'] > 10]

    # Define output filenames
    short_dep_file = file_path.replace(".txt", "_short.txt")
    medium_dep_file = file_path.replace(".txt", "_medium.txt")
    ...
    medium_dep.to_csv(medium_dep_file, sep='\t', index=False, header=False, mode='w')
    ...
    print(f"Medium-range dependencies saved to {medium_dep_file}")
    print(f"Long-range dependencies saved to {long_dep_file}")

# Example usage
file_path = "/Users/radhikasatish/nlu_cw1/NLU-Coursework-1-Recurrent-Neural-Networks/cw1_code_data/nluplus_cw1/data/wiki-dev.txt"
