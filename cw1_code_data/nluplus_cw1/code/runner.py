...
class Runner(object):
    ...
    def compute_loss(self, x, d):
        ...
        ##########################
        # --- your code here --- #
        d_one_hot_matrix = np.zeros((len(d), self.model.out_vocab_size))
...
if __name__ == "__main__":
    ...
    if mode == "train-np-rnn":
        ...
        # load training data
        sents = load_np_dataset(data_folder + '/wiki-train_long.txt')
        ...
        sents = load_np_dataset(data_folder + '/wiki-dev_long.txt')
    ...
    if mode == "train-np-gru":
        ...
        # Calculate fraction of lost vocabulary
        fraction_lost = fraq_loss(vocab, word_to_num, vocab_size)
        print(
            "Retained %d words from %d (%.02f%% of all tokens)\n" % (
                vocab_size, len(vocab), 100 * (1 - fraction_lost))
        ...
        sents = load_np_dataset(data_folder + '/wiki-train_short.txt')
        S_train = docs_to_indices(sents, word_to_num, 0, 0)
        X_train, D_train = seqs_to_npXY(S_train)

        # Load full development data
        sents = load_np_dataset(data_folder + '/wiki-dev_short.txt')
        S_dev = docs_to_indices(sents, word_to_num, 0, 0)
        X_dev, D_dev = seqs_to_npXY(S_dev)
        ...
        losses, acc = r.train_np(X_train, D_train, X_dev, D_dev, back_steps=lookback, learning_rate=lr, epochs=10)
